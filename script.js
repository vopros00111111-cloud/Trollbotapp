...
async function loadBalance() {    try {
...
async function sendTransfer() {
    ...
    try {        await apiRequest('/transfer', 'POST', { from_id: currentUser.id, to_username: to, amount: amount, comment: comment });
...
async function startBlackjack() {
    ...
    catch (e) { tg.showAlert('❌ Ошибка'); return; }
    bjGame.bet = bet;
...
async function stand() {
    ...
    document.getElementById('bet-controls').style.display = 'block';
    const m = document.getElementById('bj-message');
    if (ds > 21 || ps > ds) {        const win = bjGame.bet * 2;
...
async function spinSlots() {
    ...
    msg.innerText = txt;
    if (win > 0) { try { await apiRequest('/game-win', 'POST', { user_id: currentUser.id, amount: win, game: 'slots' }); } catch (e) {} }
    await loadBalance();
}
async function placeRouletteBet(choice) {
    ...
    const spinTimer = setInterval(function() {
        ...
        if (elapsed >= spinDuration) {
            ...
            if (winAmount > 0) { try { await apiRequest('/game-win', 'POST', { user_id: currentUser.id, amount: winAmount, game: 'roulette' }); } catch (e) {} }
            await loadBalance();        }
    }, spinInterval);
}
...
async function joinPokerTable(tableId) {
    ...
    } catch (e) { tg.showAlert('❌ Ошибка присоединения'); }
}
